...
#pragma aux rdmsr = "rdmsr" parm [ecx] value [edx eax]

void wrmsr(uint32_t msr, uint64_t data);
#pragma aux wrmsr = "wrmsr" parm [ecx] [edx eax]

void wrmsr32(uint32_t msr, uint32_t hi, uint32_t lo);
#pragma aux wrmsr32 = "wrmsr" parm [ecx] [edx] [eax]
...
{
	uint64_t msr = rdmsr(reg);
	wrmsr(reg, (msr & ~unset) | set);
}
